...
-- Minimal JSON implementation for testing (no external dependencies)
local json = (function()
  local function encode(val)
    ...
    elseif t == "string" then return '"' .. val:gsub('["\\]', '\\%1'):gsub('\n', '\\n'):gsub('\r', '\\r'):gsub('\t', '\\t') .. '"'
    elseif t == "table" then
      local is_array = #val > 0 and next(val, #val) == nil
      local parts = {}
      if is_array then
        for i, v in ipairs(val) do parts[i] = encode(v) end
  ...
  local function decode(str)
    ...
    local function parse_value()
      ...
      if c == '"' then
        ...
        local result = str:sub(start, pos - 1):gsub('\\"', '"'):gsub('\\\\', '\\'):gsub('\\n', '\n'):gsub('\\r', '\r'):gsub('\\t', '\t')
        pos = pos + 1
        return result
      elseif c == '{' then
        pos = pos + 1
        ...
        while true do
          ...
          elseif str:sub(pos, pos) == '}' then pos = pos + 1; break end
        end
        return result
      elseif c == '[' then
        pos = pos + 1
